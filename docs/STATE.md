...
**Épico atual:** F1-E13 concluído · próximo é F1-E14

---

## Onde estamos

O ferramental da Fase 1 está pronto, o design system também, `@gym/core` tem os tipos, as fórmulas de domínio (incluindo contraste de tema e situação do aluno) e os contratos de repositório, com os adapters de `localStorage` implementados em `apps/web` e populados com os dados de demonstração completos das três academias de [`SEED-DATA.md`](SEED-DATA.md). A primeira tela de verdade já existe: seletor de perfil, login, cadastro de aluno e de professor, modo demo e guard de rota, tudo funcional. O painel do professor tem a tela de identidade visual (F1-E07) e agora também o painel de alunos (F1-E13): dashboard com indicadores reais, lista com busca e filtro, ficha completa e cadastro. Falta a montagem/atribuição de treino e as telas de conteúdo do aluno.
...
**F1-E14 · Montagem e atribuição de treino.** Ver detalhes em [`ROADMAP.md`](ROADMAP.md#épicos):

1. Criar plano; editar nome, letra, foco, dia e duração
2. Exercícios: adicionar, editar, remover, reordenar (nome, séries, repetições, regiões sensíveis)
3. Duplicar plano
4. Atribuição: seleção múltipla de alunos, com indicação de quem já tem o plano — `workoutRepository.assign`/`unassign` já existem e já são usados por `features/gym/students/load-students.ts` para calcular a situação do aluno; a tela nova só precisa chamá-los
5. Publicar com confirmação e aviso de que o aluno recebe na hora; estado vazio para plano sem exercícios
...
Ponto de atenção: `StudentDetailSheet` (F1-E13) já tem a seção "Planos atribuídos" pronta para mostrar o resultado — hoje ela só explica que a atribuição "chega no F1-E14". Depois de implementar a atribuição ali, considere se a ficha do aluno deveria ganhar um atalho direto para atribuir (hoje o fluxo só existe a partir da tela de treino).

Depois dela, as telas de conteúdo do aluno (`E08 → E09 → E10 → E11 → E12`), conforme a ordem sugerida em [`ROADMAP.md`](ROADMAP.md#ordem-sugerida).
